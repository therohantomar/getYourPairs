  ...
  reducers: {
    ...
    categoryChanged: (state, action) => {
      if (action.payload === "all") {
        ...
        const filterBycategory = state.Products.filteredProducts.filter(
    ...
    companyChanged: (state, action) => {
      let filterCopy = state.Products.filteredProducts.filter(
    ...
    priceChanged: (state, action) => {
      if (action.payload === ">10000") {
        let filterCopy = state.Products.filteredProducts.filter(
          (product) => product.newPrice > 10000
        );
        if (filterCopy.length === 0) {
          const filterCopy = state.Products.AllProducts.filter(
            (product) => product.newPrice > 10000
          );
          state.Products.filteredProducts = filterCopy;
        } else {
    ...
    triggerSearch: (state, action) => {
      if (action.payload !== "") {
        const searchResult = state.Products.AllProducts.filter(
          (product) =>
            product.title.toLowerCase() === action.payload.toLowerCase()
        );
